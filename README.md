...
Без этих переменных локальная разработка откатывается на мок-данные. Если Supabase настроен, production-ошибка чтения не подменяется старыми mock-ценами: коммерческие маршруты работают fail-closed.
...
**Слой данных — через геттеры**, компоненты НЕ импортируют моки напрямую. Геттеры в `src/lib/products.ts` читают из **Supabase** через anon-клиент (`src/lib/supabase/client.ts`). Моки используются только локально без env; настроенный production не подменяет сбой БД устаревшим каталогом. Схема — `supabase/schema.sql`, маппинг строк БД (snake_case) ↔ домен (camelCase) — `src/lib/supabase/types.ts`.
...
- **Supabase подключён** — production-каталог, Auth, корзина и заказы работают через БД; моки используются только локально без env.
- **Checkout пишет в БД атомарно** — заказ, строки, клиент, история статуса и использование промокода фиксируются одной RPC-транзакцией; UUID попытки защищает повтор после потери ответа.
- **Корзина персистентна** — анонимная корзина хранится в БД и связана с `httpOnly` cookie.
- **Поиск и контентные маршруты подключены**, production работает на `https://phonetrade31.ru`.
- **CI/CD + деплой** — автодеплой на сервер по push в `main` (см. «Инфраструктура»).

### Пока не сделано / в планах ⏳

- **Вход без пароля по SMS** — сейчас реальный Supabase Auth использует телефон + пароль через внутренний email-маппинг; для OTP нужен отдельный SMS-провайдер.
- **Кастомная страница 404** пока не добавлена; используется стандартный ответ Next.js.
...
проверка обязательных secrets → `npm ci` → SEO-regression → critical lint → TypeScript → `npm run build` (standalone) → `rsync` в отдельную release-папку → аддитивная SQL-миграция → non-mutating PostgREST RPC readiness-probe → отдельный PM2 candidate на `127.0.0.1:3001` → блокирующий SEO smoke через SSH tunnel → reload production на `:3000` → live smoke → атомарное переключение `phonetrade-current` → `pm2 save`. Непроверенный release не занимает production-порт и не сохраняется в PM2. При ошибке или отмене после live-switch выполняется откат на предыдущий release; удалённый watchdog удаляет осиротевший candidate при потере runner. `workflow_dispatch` также ограничен веткой `main`, параллельные деплои сериализованы.
...
- Приложение: releases в `/opt/phonetrade-releases/`, ссылка на проверенный релиз `/opt/phonetrade-current`; исходная `/opt/phonetrade` сохраняется как initial rollback. PM2 запускает `ecosystem.config.js` выбранного release (production-порт 3000, candidate-порт 3001 только на loopback, автозапуск сохраняется лишь после live smoke).
...
## Согласия, cookie и аналитика (152-ФЗ)

- **Cookie-баннер** — `src/components/legal/CookieConsent.tsx` (provider в `(site)/layout.tsx`, cookie `pt_cookie_consent`, 13 мес). 3 равноценные кнопки «Принять все» / «Только необходимые» / «Настроить» (drawer с категориями, тумблеры по умолчанию OFF), лёгкое затемнение сайта при показе. **Я.Метрика и рекламные пиксели — только при согласии.** Метод `applyAll()`: формы зовут его при согласии на ПДн → cookie-согласие применяется автоматически.
- **Многоуровневый сбор аналитики** — `src/lib/analytics/track.ts`: Уровень 0 (всегда, обезличенно) → Уровень 1 (`visitor_id` + Метрика по согласию) → Уровень 2 (реклама по согласию). Боты метятся `page_views.is_bot` (UA-кейворды + UA >1000 хитов) и исключаются.
- **Дашборд `/admin/analytics/site`** считает через SQL-функции `analytics_*` (агрегация в БД, `is_bot=false`) — не сырые строки (был баг недетерминированной выборки). KPI-плитки: ярлык + дельта-чип у числа.
- **Формы** (регистрация, трейд-ин, заказ, ремонт): без галочки «принимаю оферту/политику» (избыточна) — только ☑ согласие на ПДн + ☐ маркетинг (+ ☑ правила ремонта в ремонте); оферта/политика — текстом под кнопкой. Активное согласие, без пассивного «продолжая…».
- **Юр-документы** `/privacy` + `/consent` (в `static_pages`) — под 152-ФЗ в действующей ред. (согласие отдельным документом, ПЭП, ст. 10.1, 30 дней, cookie-раздел с Я.Метрика/VK Ads Pixel).
- **Перенос в другой проект** — самодостаточный промт `COOKIE-ANALYTICS-PROMPT.md` в корне.
- **Я.Вебмастер** — verification-meta в metadata корневого `layout.tsx`.
- **Блог-счётчики** — `blog_posts.views_count`, RPC `increment_blog_views` + клиент `BlogViewPing` (1 раз/сессия, не боты); показ на главной и в статье (иконка глаза у даты).
