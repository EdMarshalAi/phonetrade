...
- **Домен без порта + HTTPS** — сайт на `:3000`; порты 80/443 заняты Supabase Kong, нужен nginx-роутинг.
...
- Приложение: `/opt/phonetrade` (standalone), PM2 (`ecosystem.config.js`, порт 3000, автозапуск). nginx нет.
- **Открыто снаружи: http://31.129.97.8:3000**

### Supabase (self-hosted, на том же сервере)
- Docker-стек в `/opt/beget/supabase`. API: `https://giwehapapi.beget.app` (Kong на 80/443).
