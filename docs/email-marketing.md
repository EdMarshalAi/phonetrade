...
## 14. ОБНОВЛЕНИЯ РЕАЛИЗАЦИИ (факт vs спека, 02–07.06.2026)

Разделы 0–13 выше — исходная спека (промт). Что реально в проде и где расходится:

- **Триггеры урезаны по требованию владельца:** удалены **«День рождения»** (§6.2 #7, и связанный cron §10.3) и **«Брошенная корзина 72ч»** — третий шаг цепочки (§6.2 #1). Брошенная корзина теперь **двухшаговая**: `abandoned_cart_1h` (legal_category `service`) + `abandoned_cart_24h` (`marketing`). Детект — `src/lib/email/abandoned-carts.ts`. **Не воссоздавать удалённые триггеры.**
- **Персистентная корзина сделана** (миграция `0020`): `carts` + `cart_items` по httpOnly-куке `pt_cart`, сегмент-вьюха `segment_cart_abandoners`. При оформлении заказа — `cancelQueuedByDedupPrefix('cart:{id}:')` (снимает недоотправленные письма брошенной корзины).
- **Юр.категории писем** (миграция `0018`): колонка `email_templates.legal_category` (`transactional`/`service`/`marketing`), тип согласия `service_optout`, страница `/account/email-preferences`. Правила (согласие, тихие часы, недельный лимит) применяет `process-queue.ts`. Подробности — `docs/promo-analytics-phone-validation.md`.
- **Шапки писем — новые баннеры** в Storage `email/headers/{welcome,order,cart,review,crosssell,campaign}.png`: Apple-стиль (iPhone 17/Pro, Cosmic Orange) через higgsfield nano-banana + логотип PT и русский заголовок (ImageMagick, шрифт SF). Кэш-бастер `?v=N` в `scripts/seed-email-templates.ts` (текущий `v4`) — при замене картинок бампить и пере-сеять.
- **Редактор/кампании — UX:** поле «Картинка-хедер» = `ImageField` (загрузка **файлом ИЛИ по ссылке**, превью 16:9, бакет `product-images`/`email/headers`) — не голый URL-input. Под полем «Текст» — кнопка «Переменные» (`marketing/_components/VariablesButton` + реестр `src/lib/email/variables.ts`), вставка `{{…}}` в позицию курсора. В кампаниях список переменных сужен (клиент/промо/служебные — без `order.*`/`cart.*`, которых в массовой рассылке нет).
- **Демо-данные в тестах:** тест-отправка (`testSendTemplate`/`testCampaign`) и превью используют `sampleVars` (имя «Денис», заказ `PT-2026-0042`). Тест шлёт на один адрес с темой `[ТЕСТ]` и **НЕ логируется** в `email_sends_log`. Реальные имена подставляются на рантайме (welcome — `input.name`, кампания — `r.name` каждого получателя) — «Денис» в боевую рассылку попасть не может.
- **Из спеки НЕ реализованы:** open/click-tracking (§13 tracking pixel / click redirect), double opt-in (§8.4), дни рождения (§10.3 — удалены).
