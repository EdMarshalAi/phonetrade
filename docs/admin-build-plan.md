...
## 5b. Что осталось (heavy, требуют ревью на проде)
- ⬜ Варианты товара (память/цвет) + мульти-галерея (расширение формы товара + таблицы product_variants/product_images)
- ⬜ Ручное создание заказа в админке; конвертация lead→заказ
- ⬜ **Запись заказов из корзины в БД + Telegram при новом заказе** (трогает публичный checkout `CartShell` — делать аккуратно с верификацией)
- ⬜ TradeIn/Bento публичная отрисовка из БД; реальный Supabase Auth для публичного кабинета (сейчас localStorage-мок)
