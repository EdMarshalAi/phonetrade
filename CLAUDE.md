...
**Сервер.** `root@31.129.97.8` (Ubuntu 24.04), ключ `~/.ssh/beget_phonetrade`. Приложение в `/opt/phonetrade` (standalone), запущено через PM2 (`ecosystem.config.js`, порт 3000, автозапуск включён). nginx НЕ установлен. **Сайт доступен снаружи: http://31.129.97.8:3000** (домен без порта пока не настроен — порты 80/443 заняты Supabase Kong).

**Supabase.** Self-hosted Docker-стек в `/opt/beget/supabase` на том же сервере. Публичный API: `https://giwehapapi.beget.app` (Kong на 80/443). Postgres снаружи через pooler `31.129.97.8:5432`, логин `postgres.your-tenant-id`. Креды — в `/opt/beget/supabase/.env` на сервере (и в локальном `.env.local`). Прямое управление БД: `ssh root@31.129.97.8 "docker exec -i supabase-db psql -U postgres -d postgres"`.
...
Реальный Supabase Auth (сейчас мок на localStorage); запись заказов из корзины в БД (таблицы готовы); персистентность корзины; поиск (поле в хедере есть, страницы результатов нет); домен без порта + HTTPS (нужен nginx-роутинг рядом с Supabase Kong). Многие маршруты из хедера/футера ещё ведут в 404: `/about`, `/blog`, `/delivery`, `/warranty`, `/trade-in`, `/loyalty`, `/contacts`, `/used`, `/catalog` и юридические страницы. Детальный статус — в `README.md` → «Статус реализации».
