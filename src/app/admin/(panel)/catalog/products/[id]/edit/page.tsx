...
export default async function EditProductPage({
  ...
  const categoryPricing = Object.fromEntries(
    (cats ?? []).map((c) => [c.slug as string, { markup_percent: Number(c.markup_percent ?? 10), min_margin_rub: Number(c.min_margin_rub ?? 0), card_markup_percent: Number(c.card_markup_percent ?? 15) }])
  );
